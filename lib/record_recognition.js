...
record_recognition.record_start_or_resume = function(stream, data){
    ...
    existing_stream[event_id + short_split_id] = stream;


    save_startdata(data);

    stream.on('end', ()=>{
      if(existing_stream[event_id + short_split_id]){
        existing_stream[event_id + short_split_id] = null;
        delete existing_stream[event_id + short_split_id];
      }
    ...
    remoteWriteStream.on('finish', ()=>{
      console.log("remoteWriteStream finish", data.short_split_id);
...
record_recognition.record_suspend = function(data){
  ...
  check_existing_stream_and_stop(data);
...
function check_existing_stream_and_stop(data){
    ...
    setTimeout(()=>{
      if(existing_stream[event_id + short_split_id]){
        existing_stream[event_id + short_split_id].close();
        delete existing_stream[event_id + short_split_id];
      }
    },each_speech_duration + 10*1000);
}
